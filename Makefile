...
MYSQL_PASSWORD?=medical-app## Do `make <target> ... MYSQL_PASSWORD=<something> ...` to define the database password
...
clean/docker: ## Delete all docker images used by medical-app (and stop according running container)
	for image in $$($(DOCKER) images -q --filter reference="medical-app-dev*:*" --format '{{.ID}}'); do \
